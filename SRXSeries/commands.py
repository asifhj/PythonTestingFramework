...
class Commands:
    ...
    def get_ch_alarm_data(self):
        ...
        i = 0
        for record in soup.findAll("alarm-summary"):
            children = record.findChildren()
            self.ch_alarm_data[i] = {}
            for child in children:
                self.ch_alarm_data[i][child.name.strip().replace("-", "_")] = child.text.strip()
                self.ch_alarm_data[i]["chassiname"] = chassisname
            i += 1
        #print json.dumps(self.ch_alarm_data, indent=4)
   ...
   # For SRX display xml
    ...
    def get_sys_cores_data(self):
        ...
        if not output:
            with open(self.file_name, "rb") as fopen:
                for line in fopen:
                    if not re.match(".*@.*>\s+show\s+system\s+core\-dumps\s+.\s+display.*", line, re.M | re.I) == None:
                        output_type = "xml"
                        break
                for line in fopen:
                    if not line.strip() == "":
        ...
        if output_type == "xml":
            soup = BeautifulSoup(output, "lxml")
            for line in soup.findAll("output"):
                #print lineself.re_data[i][child.name.strip().replace("-", "_")] = child.text.strip()
                m = re.match(r'(?P<chassisname>[sfc[0-9]+.*:|lcc[0-9]+.*:|fpc[0-9]+.*:])', line.text.strip(), re.M | re.I)
                if m:
                    chassisname = m.groups(0)[0]
                m = re.match(r'.*\s(?P<user>[a-zA-Z]+)\s+[a-zA-Z]+\s+(?P<size>[0-9]+)\s+(?P<date>.*)(?P<location>[a-zA-Z0-9]*\s+\/.*)', line.text.strip(), re.M | re.I)
                if m:
    ...
    def get_sys_stats_data(self):
        ...
        for line in output:
            if line.strip():
                ...
                m = re.match(r'(?P<chassisname>[sfc[0-9]+.*:|lcc[0-9]+.*:|fpc[0-9]+.*:])', line, re.M | re.I)
                if m:
                    chassisname = m.groups(0)
                m = re.match(r'(\d+)\s+packets discarded waiting for resolution', line.strip(), re.M | re.I)
                if m:
                    self.sys_stats_data["arpdiscarded"]=m.groups()[0]
                m = re.match(r'(\d+)\s+requests for memory denied', line.strip(), re.M | re.I)
                if m:
                    self.sys_stats_data["arpdenied"]=m.groups()[0]
        #print json.dumps(self.sys_stats_data, indent=4)
